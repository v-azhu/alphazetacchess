...
### V0.9.6 — Self-Play Policy Distillation — COMPLETE (negative generation-0 result)

Direct follow-through on `docs/v0.9.4.md`'s "Round 2" conclusion: the more promising
untried direction was training a policy toward `MCTSEngine`'s *own* search result
(real AlphaZero-style self-distillation) rather than an external engine's independent
opinion. New `MCTSEngine.search_root`/`root_visit_distribution` expose the root's full
move -> visit-count distribution (`choose_move` now built on top of `search_root`,
completely unchanged behavior, verified by the full suite). New
`tools/generate_mcts_policy_labels.py` plays `MCTSEngine` against itself and records
each position's most-visited move in the *same* label format the Pikafish pipeline
uses, so `tools/train_policy_network.py` needed zero changes. Unlike the Pikafish
rounds, this needs no external engine or user-local setup -- generated **5,263 real
self-play labels** (65 games) entirely in-session. 6 new tests. Full suite: **319/319
green.**

Trained on this data: validation accuracy plateaued at **28.90%** (lower than either
Pikafish round's 43.9%/47.1% -- a 200-sim MCTS search is a noisier "expert" than
Pikafish, so labels carry less learnable signal). Real strength results: 45.0% (20
games) vs. uniform priors, roughly even like both Pikafish rounds; **15.8% (19 games)
vs. V0.9.3's heuristic priors**, clearly losing -- the same qualitative pattern all
three rounds now show. Notably, the self-play data was generated by heuristic-prior
MCTS itself (V0.9.5's new default), so this network lost to the very search
configuration that produced its own training labels -- expected for a true
generation-0 distillation attempt (lossy compression of a weak search's opinion into a
small classifier with too little data to preserve it), not a surprising failure.

**Recommendation, across all three rounds now: stop incremental investment in the
policy-network direction.** Two genuinely different training signals, three real
attempts, none beating V0.9.3's heuristic priors. Continuing would need either a much
larger multi-generation self-play bootstrap (a real multi-session undertaking) or a
different approach entirely -- neither is a natural next increment. This is a
deliberate stopping point, not an unfinished thread; all the infrastructure
...
Current hand-off:
    ...
    *why* calibrated material underperforms (a depth=3 re-run, or
    ...
    of whichever is picked.
        ↓
    Picked (e) -- self-play-refined policy targets -- scoped as a
    single generation-0 round rather than a full multi-generation
    bootstrap. V0.9.6 (see the V0.9 section above and docs/v0.9.6.md):
    new MCTSEngine.search_root/root_visit_distribution expose the
    root's full visit distribution; new tools/generate_mcts_policy_
    labels.py plays MCTSEngine against itself and records each
    position's own most-visited move as a label, same format the
    Pikafish pipeline uses so train_policy_network.py needed no
    changes. Unlike the Pikafish rounds this needed no external
    engine/user-local step -- generated 5,263 real labels (65 games)
    entirely in-session. 319/319 green throughout.
        ↓
    Trained: 28.90% validation accuracy (lower than either Pikafish
    round -- a 200-sim search is a noisier "expert" than Pikafish).
    Real strength: 45.0% vs. uniform (roughly even, like both Pikafish
    rounds), 15.8% vs. V0.9.3's heuristic priors (clearly losing, same
    pattern all three rounds now show). Notable: the label source
    (heuristic-prior MCTS, V0.9.5's new default) IS the baseline this
    network lost to -- expected for a lossy generation-0 distillation
    with too little data, not a surprising failure.
        ↓
    THREE rounds now (2 Pikafish-imitation, 1 self-distillation), two
    genuinely different signals, none beating V0.9.3's heuristic
    priors. Recommendation: STOP incremental investment in the
    policy-network direction -- continuing needs either a real
    multi-generation self-play bootstrap (a genuine multi-session
    undertaking) or a different approach, neither a natural next
    increment. Deliberate stopping point, not unfinished -- all
    infrastructure remains available for whoever picks a larger-scale
    attempt back up. docs/v0.9.6.md has the full three-round comparison
    table.
        ↓
    Next: (c) V0.8.3's MVV-LVA question remains open at n=57 (low
    priority, per the note above). The policy-network thread is
    considered settled for now absent a decision to commit to a much
