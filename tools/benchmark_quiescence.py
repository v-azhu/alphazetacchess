...
def run_case(board, color, depth, use_quiescence):
    engine = SearchEngine(depth=depth, use_quiescence=use_quiescence, use_null_move=False)
    ...
    return {"time": elapsed, "nodes": result.nodes_evaluated, "nps": nps, "score": result.score, "move": move_key(result.best_move), "state_ok": state_ok}

def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--depths", nargs="+", type=int, default=[2, 3, 4], help="search depths to benchmark (default: 2 3 4)")
    ...
    for name, fen, color in POSITIONS:
        ...
        for depth in depths:
            ...
            print(
                "d{}: OFF time={:.3f}s nodes={} nps={:.1f} score={} move={} state_ok={} | "
                "ON time={:.3f}s nodes={} nps={:.1f} score={} move={} state_ok={} | "
                "move_same={} score_delta={:+d}".format(
                    depth, off["time"], off["nodes"], off["nps"], off["score"], off["move"], off["state_ok"],
                    on["time"], on["nodes"], on["nps"], on["score"], on["move"], on["state_ok"], move_same, score_delta,
                )
            )
